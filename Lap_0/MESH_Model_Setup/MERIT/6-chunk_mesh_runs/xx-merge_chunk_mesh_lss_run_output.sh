...
# === Filename-variable pairs ======
filenames=(RFF_H_GRD.nc)
variables=(RFF)
#filenames=(RFF_H_GRD.nc ET_D_GRD.nc RFF_D_GRD.nc SNO_D_GRD.nc LQWSSNO_D_GRD.nc ALWSSOL_D_IG1_GRD.nc ALWSSOL_D_IG2_GRD.nc ALWSSOL_D_IG3_GRD.nc ALWSSOL_D_IG4_GRD.nc)
#variables=(RFF ET RFF SNO LQWSSNO ALWSSOL ALWSSOL ALWSSOL ALWSSOL)
...
def clean_string_attrs(obj):
    """Recursively strip trailing whitespace from string attributes."""
    if hasattr(obj, 'attrs') and obj.attrs:
        for key, val in list(obj.attrs.items()):
            if isinstance(val, str):
                obj.attrs[key] = val.rstrip()
    # Recurse on variables and coords
    for var in list(obj.data_vars + obj.coords):
        clean_string_attrs(obj[var])
    return obj
...
# Clean string attributes to remove trailing spaces/padding
stitched = clean_string_attrs(stitched)

if "time" in stitched.variables:
    # Strip any potential padding from time attrs too
    time_units = datasets[0]["time"].attrs.get("units", "").rstrip()
    time_calendar = datasets[0]["time"].attrs.get("calendar", "standard").rstrip()
    stitched["time"].attrs["units"] = time_units
    stitched["time"].attrs["calendar"] = time_calendar
